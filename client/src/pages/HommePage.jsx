...
function HommeCard({ product, index }) {
  const { add } = useCart()
...
export default function HommePage() {
  ...
  const hommeProducts = explicit.length ? explicit : products.filter((product) => product.categorySlug === 'boubou')
      ...
      <section className="max-w-7xl mx-auto px-4 mt-8">
        {loading ? (
          ...
          <div className="rounded-[2rem] bg-white px-6 py-20 text-center shadow-[0_18px_50px_rgba(140,98,57,0.08)]">
            <p className="text-lg font-medium text-[#8C6239]">Aucun article trouve dans la section Homme</p>
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {hommeProducts.map((product, index) => (
              <HommeCard key={product.id} product={product} index={index} />
            ))}
          </div>
        )}
      </section>
